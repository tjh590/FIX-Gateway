...
def main_setup():
    ...
    parser.add_argument(
        "--daemonize", "-D", action="store_true", help="Run program in the background"
    )
    parser.add_argument(
        "--config-file",
    ...
    if environ.get("INVOCATION_ID", False):
        ...
        if not config.get("auto start", False):
            os._exit(0)
    ...
    if "connections" in config:
        for con in config["connections"]:
            if "module" in config["connections"][con]:
                if (
                    config["connections"][con]["module"].lower()
                    == "fixgw.plugins.quorum"
                ):
    ...
    if args.playback_start_time:
        ...
        for each in config["connections"]:
            if config["connections"][each]["module"] == "fixgw.plugins.data_recorder":
                ...
                path = os.path.join(
                    config["connections"][each]["filepath"].format(CONFIG=config_path),
                    args.playback_start_time.strftime("%Y"),
                    args.playback_start_time.strftime("%m"),
                    args.playback_start_time.strftime("%d"),
                )
                filepath = os.path.join(
                    path, args.playback_start_time.strftime("%Y-%m-%d.%H.json")
                )
                print(filepath)
                file_list = [filepath]
                if not os.path.isfile(filepath):
                    raise Exception("No logs found for the date and time provided")
                ...
                while more_files:
                    more_path = os.path.join(
                        config["connections"][each]["filepath"].format(
                            CONFIG=config_path
                        ),
                        next_hour.strftime("%Y"),
                        next_hour.strftime("%m"),
                        next_hour.strftime("%d"),
                    )
                    more_file = os.path.join(
                        more_path, next_hour.strftime("%Y-%m-%d.%H.json")
                    )
